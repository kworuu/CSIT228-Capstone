...
public class ManageSupplyRequestsController {
    ...
    @FXML private TableColumn<SupplyRequest, String> colHistoryItem;
    @FXML private TableColumn<SupplyRequest, String> colHistoryCenter;
    ...
    private void setupTables() {
        ...
        colStagingItem.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getItemName()));
        colStagingQty.setCellValueFactory(c -> new SimpleObjectProperty<>(c.getValue().getQuantityRequested()));
        colStagingUnit.setCellValueFactory(c -> new SimpleStringProperty(c.getValue().getUnit()));

        tableHistory.setItems(historyItems);
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("MMM dd, yyyy");

        colHistoryDate.setCellValueFactory(c ->
                new SimpleStringProperty(c.getValue().createdAt() != null ? c.getValue().createdAt().format(dtf) : ""));

        colHistoryItem.setCellValueFactory(c ->
                new SimpleStringProperty(c.getValue().quantity() + "x " + c.getValue().itemName()));

        colHistoryCenter.setCellValueFactory(c ->
                new SimpleStringProperty(c.getValue().targetCenterName() != null ? c.getValue().targetCenterName() : ""));

        colHistoryStatus.setCellValueFactory(c ->
                new SimpleStringProperty(c.getValue().status() != null ? c.getValue().status().displayLabel() : ""));
    }
    ...
    private void handleSubmitRequest() {
        ...
        try {
            ...
            String brgyName = SessionContext.current().getUser().displayName();
            EvacuationCenter targetCenter = comboBoxCenter.getValue();
            Long targetCenterId = targetCenter != null ? targetCenter.id() : null;

            for (SupplyRequestItem stagedItem : stagingItems) {
                // Notice we now pass targetCenterId properly!
                SupplyRequest newRequest = new SupplyRequest(
                        0,
                        stagedItem.getItemId(),
                        null, // itemName (DB will ignore this on insert)
                        targetCenterId,
                        null, // targetCenterName
            ...
            Alert alert = new Alert(Alert.AlertType.INFORMATION, "Supply request submitted to LGU successfully!");
            alert.showAndWait();
